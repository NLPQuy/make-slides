...
% \makebox, NOT {\centering ...\par}, and not \begin{center} either. A centering group that is
% the FIRST thing in a frame body silently drops its contents: the frame compiles, the checker
% passes, and the slide ships blank. Measured, seven cases:
%   bare tabular              -> renders
%   {\centering T\par} first   -> VANISHES
%   {\centering T} first       -> VANISHES
%   \begin{center}T\end{center} -> renders (but costs ~20pt of unbudgeted space)
%   any text, then {\centering T\par} -> renders
%   \takeaway, then {\centering T\par} -> renders
%   \makebox[\textwidth]{T}    -> renders
% So the deck-wide rule "{\centering ...\par}, never \begin{center}" holds everywhere EXCEPT as
% the first element of a frame body. There, use \makebox.
\begin{frame}{Outline}
